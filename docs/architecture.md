...
All monitor and config code is guarded by `import "server-only"` (see [GetErrorMonitor.ts:1](../src/lib/errorMonitor/GetErrorMonitor.ts#L1)). If a client component ever imports it by mistake, the build fails. Tokens never reach the bundle.
...
The home page is a **Server Component** ([src/app/page.tsx](../src/app/page.tsx)) that reads the project list from Strapi, picks the first project, prefetches all panel queries server-side, and hydrates the client. The kiosk displays data on the first paint without a client-side fetch round-trip.

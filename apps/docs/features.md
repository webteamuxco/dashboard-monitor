...
Every data feature is parameterized by the Strapi **`documentId`** of the active project, threaded down from [DashboardContent](../src/app/features/dashboard/ui/DashboardContent.tsx) via [useActiveProject](../src/app/features/dashboard/hooks/useActiveProject.ts). It is part of every query key, so switching project refetches everything without manual invalidation.
...
- **Backed by:** [src/lib/config/](../src/lib/config/) (`StrapiClientFactory` → `StrapiClientStrategy` → `StrapiRepository`)
...
Both queries are seeded server-side with `setQueryData` in [page.tsx](../src/app/page.tsx), so the selector and the first project's settings are available on first paint.
...
8. **Wire it into [page.tsx](../src/app/page.tsx)** (prefetch) and into `DashboardContent` (render).

...
**`/api/blocks/[blockId]?tag=` names one of the element's own tags, never a query.** The route forwards the raw param and the **log strategy** matches it against `strategy.tags`, throwing when it matches none: nothing the browser sends ever reaches the provider query verbatim. Omitting the param asks for **every** declared tag, which the strategy reads as one query per tag — never as one ANDed query, whose terms would ask for the logs carrying every service at once. The block measure then returns one series per tag, which is what a `stackedBar` stacks; a `bar` passes a `tag` to narrow that down to one.
